...
async def run_mcp_agent_defended(user_prompt: str, llm_tools: list,
    ...
    """返回 (reply, attack_blocked, defense_triggered)"""
    ...
    defense_triggered = {
        "middleware": {"triggered": False, "details": ""},
        "server": {"triggered": False, "details": ""},
        "intent": {"triggered": False, "details": ""},
    }
                    ...
                    for tc in assistant_message.tool_calls:
                        ...
                        if func_name == "get_reviews":
                            if not skip_clean:
                                ...
                                log_warning("响应清洗已被跳过 (--skip-clean)")
...
def main():
    ...
    active_layers = []
    if not skip_clean:
        active_layers.append(f"  1. {Colors.MAGENTA}Client 端响应清洗{Colors.RESET} — 正则过滤恶意指令模式")
    else:
        active_layers.append(f"  1. {Colors.RED}Client 端响应清洗 [已禁用]{Colors.RESET}")
    if not skip_server_defense:
        active_layers.append(f"  2. {Colors.YELLOW}Server 端最小权限{Colors.RESET} — mcp_server.py --defense")
    else:
        active_layers.append(f"  2. {Colors.RED}Server 端最小权限 [已禁用]{Colors.RESET}")
    if not skip_intent:
        active_layers.append(f"  3. {Colors.CYAN}双重意图验证{Colors.RESET} — 独立校验 Agent 判断用户意图")
    else:
        active_layers.append(f"  3. {Colors.RED}双重意图验证 [已禁用]{Colors.RESET}")
    print("防御机制:")
    ...
    layer_names = [
        ("middleware", "响应清洗 (Middleware)", Colors.MAGENTA),
        ("server", "Server 端最小权限", Colors.YELLOW),
        ("intent", "双重意图验证", Colors.CYAN),
    ]
    ...
    for key, name, color in layer_names:
        ...
        if d["triggered"]:
            ...
            status = f"{Colors.YELLOW}⏭️  未触发 (前面防御已拦截){Colors.RESET}"
    ...
    if attack_triggered:
        ...
        print(f"   三层防御中有至少一层成功阻止了攻击。{Colors.RESET}")
    else:
